...
def post(day=None):
    match day or datetime.date.today().weekday():
        ...
        case 6:
            blogger = HistoricBlogger(
                "Ты - математик", 
                "Расскажи про одно рандомное матиматическое открытие и о том, кто его открыл. Используй смайлики и не более 150 слов. Не используй 'Окей' и 'Конечно'",
                ['#открытия', '#первые', '#математика', '#лучшие']
            )
    blogger.post()
