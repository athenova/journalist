...
def post(day=None):
    ...
    index = day_count // 7
    system_prompt=f"Ты - начинающий атлет, мужчина, блогер с 1000000 подписчиков. У тебя лишний вес, отличное чувство юмора и позитивный настрой по жизни. Это твой {day_count} день в тренажёрном зале. Ты тренируешься, чтобы есть, и ешь, чтобы тренироваться" 
    word_count = 150
    match day or (datetime.date.today().weekday() + 6) % 7:
        case 0:
            tasks = json.load(open("./files/sports/gym_couch/gym_couch1.json", "rt", encoding="UTF-8"))
        ...
        case 2:
            tasks = json.load(open("./files/sports/gym_couch/gym_couch3.json", "rt", encoding="UTF-8"))
            index = index % len(tasks)
            task = tasks[index]
            blogger = GymCouch(
                system_prompt, 
        ...
        case 3:
            tasks = json.load(open("./files/sports/gym_couch/gym_couch4.json", "rt", encoding="UTF-8"))
            index = index % len(tasks)
            task = tasks[index]
            blogger = GymCouch(
                system_prompt, 
        ...
        case 5:
            tasks = json.load(open("./files/sports/gym_couch/gym_couch6.json", "rt", encoding="UTF-8"))
            index = index % len(tasks)
            task = tasks[index]
            blogger = GymCouch(
                system_prompt, 
