...
class CtoBlogger(Journalist):
    def _message_generator(self):
        return DeepSeekTextGenerator('Ты - технический директор, лидер команды со 100% харизмой, всегда оптимистично настроенный и с отличным чувством юмора')
    ...
    def _prompt_constructor(self, _):
        return f"Опиши проблему '{self.task['problem']}' из категории '{self.task['category']}', как если бы рассказывал другу, выбери метод решения, опиши метод решения, используй смайлики, используй менее 150 слов"

    def __init__(self, task):
        self.task = task
        tagadder = TagAdder(['#cto', '#it', '#ит', '#айти', '#проблемы', '#решения', f"#{task['category']}"])
        posters = [
...
def post(offset=0):
    ...
    tasks = json.load(open("./files/profession/cto_in_fire.json", "rt", encoding="UTF-8"))
    index = ((today - start_date).days + offset) % len(tasks)
    blogger = CtoBlogger(tasks[index])
    blogger.post()
